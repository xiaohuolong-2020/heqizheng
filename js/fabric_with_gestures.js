...
fabric.Collection = {
  ...
  remove: function() {
    ...
    for (var i = 0, length = arguments.length; i < length; i++) {
      ...
      if (index !== -1) {
		//当前删除的元素
		var cur = arguments[i]._element.currentSrc.split("element")[1].split(".")[0];
		var num = elemtArr.indexOf(cur);
		//从数组中删除当前删除的元素
		elemtArr.splice(num,1);
  ...
  fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
    ...
    _drawControl: function(control, ctx, methodName, left, top) {
      ...
      if (this.isControlVisible(control)) {
          if(control=='br'){
            ctx.drawImage($("#deleteIcon")[0],left-size*.2,top-size*.2,size*1.5,size*1.5);
            return;
        }
		if(control=='mtr'){
            ctx.drawImage($("#refreshIcon")[0],left-size*.2,top-size*.2,size*1.5,size*1.5);
            return;
        }
